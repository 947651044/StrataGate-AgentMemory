  ...
  it('loads into the official Cordis services and registers the complete memory protocol', async () => {
    ...
    try {
      ...
      await expect(profileUpdate!.execute({ field: 'userPreferredName', value: '橙子' }, { agent, callId: 'profile-negative-name' } as never))
        .rejects.toThrow(/explicit current user request/)
      conversationMessages.push({ id: 'profile-user-negative-named', role: 'user', source: { kind: 'user' }, content: [{ type: 'text', text: '不要把 preferredLanguage 改为英文' }] })
      await expect(profileUpdate!.execute({ field: 'preferredLanguage', value: '英文' }, { agent, callId: 'profile-negative-named' } as never))
        .rejects.toThrow(/explicit current user request/)
      conversationMessages.push({ id: 'profile-user-negative-clear', role: 'user', source: { kind: 'user' }, content: [{ type: 'text', text: '不要清空 preferredLanguage' }] })
      await expect(profileUpdate!.execute({ field: 'preferredLanguage', value: '' }, { agent, callId: 'profile-negative-clear' } as never))
        .rejects.toThrow(/explicit current user request/)
      conversationMessages.push({ id: 'profile-user-positive-named', role: 'user', source: { kind: 'user' }, content: [{ type: 'text', text: '请把 preferredLanguage 改为英文' }] })
      expect(await profileUpdate!.execute({ field: 'preferredLanguage', value: '英文' }, { agent, callId: 'profile-positive-named' } as never))
      ...
      await expect(profileUpdate!.execute({ field: 'userBackground', value: '工程师' }, { agent, callId: 'profile-fact' } as never))
        .rejects.toThrow(/explicit current user request/)
      conversationMessages.push({ id: 'profile-user-one-turn-language', role: 'user', source: { kind: 'user' }, content: [{ type: 'text', text: '用英文回答。' }] })
      await expect(profileUpdate!.execute({ field: 'preferredLanguage', value: '英文' }, { agent, callId: 'profile-one-turn-language' } as never))
        .rejects.toThrow(/explicit current user request/)
      conversationMessages.push({ id: 'profile-user-one-turn-reply', role: 'user', source: { kind: 'user' }, content: [{ type: 'text', text: '回复收到。' }] })
      await expect(profileUpdate!.execute({ field: 'responsePreferences', value: '收到' }, { agent, callId: 'profile-one-turn-reply' } as never))
        .rejects.toThrow(/explicit current user request/)
      ...
      expect(await profileUpdate!.execute({ field: 'userPreferredName', value: '橙子' }, { agent, callId: 'profile-name' } as never))
        .toMatchObject({ modified: true })
      conversationMessages.push({ id: 'profile-user-name-with-you', role: 'user', source: { kind: 'user' }, content: [{ type: 'text', text: '你以后叫我橙子' }] })
      await expect(profileUpdate!.execute({ field: 'assistantPreferredName', value: '我橙子' }, { agent, callId: 'profile-name-with-you-wrong-field' } as never))
        .rejects.toThrow(/explicit current user request/)
      expect(await profileUpdate!.execute({ field: 'userPreferredName', value: '橙子' }, { agent, callId: 'profile-name-with-you' } as never))
        .toMatchObject({ modified: false })
      conversationMessages.push({ id: 'profile-user-name-with-hope', role: 'user', source: { kind: 'user' }, content: [{ type: 'text', text: '我希望你以后叫我橙子' }] })
      await expect(profileUpdate!.execute({ field: 'assistantPreferredName', value: '我橙子' }, { agent, callId: 'profile-name-with-hope-wrong-field' } as never))
        .rejects.toThrow(/explicit current user request/)
      ...
      expect(await profileUpdate!.execute({ field: 'assistantPreferredName', value: '小橙' }, { agent, callId: 'profile-assistant-name' } as never))
        .toMatchObject({ modified: true })
      conversationMessages.push({ id: 'profile-user-remember-name', role: 'user', source: { kind: 'user' }, content: [{ type: 'text', text: '记住，以后叫我橙子' }] })
      expect(await profileUpdate!.execute({ field: 'userPreferredName', value: '橙子' }, { agent, callId: 'profile-remember-name' } as never))
        .toMatchObject({ modified: false })
      conversationMessages.push({ id: 'profile-user-remember-assistant-name', role: 'user', source: { kind: 'user' }, content: [{ type: 'text', text: '记住，你以后叫小橙。' }] })
      expect(await profileUpdate!.execute({ field: 'assistantPreferredName', value: '小橙' }, { agent, callId: 'profile-remember-assistant-name' } as never))
        .toMatchObject({ modified: false })
      ...
      conversationMessages.push({ id: 'profile-assistant-negative-proposal', role: 'assistant', source: { kind: 'model' }, content: [{ type: 'text', text: '我不同意将 preferredLanguage 改为“英文”。如果同意，请回复“同意”。' }] })
      conversationMessages.push({ id: 'profile-user-negative-proposal-consent', role: 'user', source: { kind: 'user' }, content: [{ type: 'text', text: '同意' }] })
      await expect(profileUpdate!.execute({ field: 'preferredLanguage', value: '英文' }, { agent, callId: 'profile-negative-proposal-consent' } as never))
        .rejects.toThrow(/direct subsequent 同意/)
      conversationMessages.push({ id: 'profile-assistant-unrelated-field', role: 'assistant', source: { kind: 'model' }, content: [{ type: 'text', text: '当前 userPreferredName 为橙子。要把助理称呼改为“小橙”吗？请回复“同意”。' }] })
      conversationMessages.push({ id: 'profile-user-unrelated-field-consent', role: 'user', source: { kind: 'user' }, content: [{ type: 'text', text: '同意' }] })
      await expect(profileUpdate!.execute({ field: 'userPreferredName', value: '小橙' }, { agent, callId: 'profile-unrelated-field-consent' } as never))
        .rejects.toThrow(/direct subsequent 同意/)
