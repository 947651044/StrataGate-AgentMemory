...
const SUMMARIZER_PARAMETERS: ParameterSchemaSpec = {
  ...
  l0Tags: { ...STRING_ARRAY, description: 'L0: a few distinctive retrieval labels (usually 2-5) for named people, projects, organizations, tools, technical topics, or specific task concepts; omit generic labels such as discussion, conversation, problem, or task.', required: true },
  ...
  shouldExtract: { type: 'boolean', description: 'True only if this Block contains at least one evidence-supported item worth preserving as a long-term Event (for example a decision, stable preference, meaningful project change or result, important failure, useful fact, or open item to track). False for greetings, repeated confirmations, transient process noise, or facts without future memory value.', required: true },
}
...
Set shouldExtract to true only if this Block has at least one evidence-supported item with future memory value worth a long-term Event: a clear decision, stable preference, material project change, meaningful task result, important failure and cause, future-useful fact, or open item worth tracking. A fact's mere presence is insufficient. Set it to false for greetings, repeated confirmations, low-value process details, and one-off execution noise. An assistant's unsupported suggestion or old-memory recap alone does not qualify.
